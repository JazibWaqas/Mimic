...
class VaultReport(BaseModel):
    ...
    friction_log: List[str] = Field(..., description="Temporal awareness: how confidence evolved")
    post_mortem: Dict[str, Any] = Field(..., description="Global strengths, weaknesses, and responsibility")
    next_steps: List[str] = Field(..., description="Actionable prescriptions for the user")
    technical: List[str] = Field(..., description="Behind-the-scenes technical discipline log")
